...
interface RepositoryService {
  ...
  /*  @GET("/api/v1/pokedex/1")
    fun getRepository(callback: Callback<Pokedex>)*/

    @Headers("Accept: application/json; charset=utf-8", "Accept-Language: en")
    @GET("/users/google/repos?per_page=20&page=1")
    fun getRepository(callback: Callback<List<Repository>>)

    @GET("/{resource_uri}")
    fun getRepositoryDetails(@Path(value = "resource_uri", encode = false) resourceUri: String, callback: Callback<Repository>)
}
